...
public class MovieDAODerbyImpl extends GenericDAODerbyImpl implements MovieDAO {
    ...
    @Override
    public List<MovieDTO> findAllNowShowing() {
        List<MovieDTO> movies = new ArrayList<MovieDTO>();
        
        try {
            PreparedStatement allMoviesQ = conn.prepareStatement("SELECT title, release_date, age_rating, director, actors, synopsis "
                    + "FROM movies WHERE release_date < current_date");
            ResultSet allMovies = allMoviesQ.executeQuery();
